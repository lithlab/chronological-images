...
- Use `<button>` for actions, `<a>`/`<Link>` for navigation (not `<div>`)
- Images need `alt` (or `alt=""` if decorative)
- Decorative icons need `aria-hidden="true"`
- Async updates (toasts, validation) need `aria-live="polite"`
- Use semantic HTML (`header`, `nav`, `main`, `footer`) before ARIA
...
- SVG: transforms on a wrapper with `transform-box: fill-box; transform-origin: center`
