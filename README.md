...
仓库名是 `chronological-images`。页面上的产品名仍是「做本画册」。浏览器里已有的画册存在 localStorage（键名仍是 `lithlab.html003.v1` 等），换文件夹名不会丢掉本机已有数据。

## 需要什么

- [Python 3](https://www.python.org/downloads/)
- 任选一家生图服务的 API Key（火山方舟 / 通义 / 硅基流动 / Gemini / OpenAI）

作者这边目前主要跑通了**火山方舟**；页面里也有其他厂商的填写位置，但那些 Key 尚未完整实测。
...
- 不要把 Key 写进代码、README 或 `.env` 后 `git add`。
- 生成图会落在 `processed/`，该目录已被 `.gitignore` 忽略，不要把里面的图提交上去。
